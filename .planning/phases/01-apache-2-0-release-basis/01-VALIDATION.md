...
| 01-01-02 | 01 | 1 | LIC-02 | T-02 | Canonical license text and factual NOTICE ship unchanged. | release check | `cmp -s LICENSE ../agentic-operator-core/LICENSE && test "$(cat NOTICE)" = $'AgentGate\nCopyright 2026 Clawdlinux.'` | No, Wave 0 | pending |
| 01-01-02 | 01 | 1 | LIC-03 | T-03 | Only the 4 locked Go files receive Apache headers. No Go BSL text remains. | release check plus regression | `test "$(git ls-files '*.go' | xargs rg -l 'Licensed under the Apache License, Version 2\.0\.' | sort | wc -l | tr -d ' ')" = 4 && ! git ls-files '*.go' | xargs rg -n 'Business Source License|Business Source|BSL' && go test ./cmd/agentgw ./internal/gateway ./internal/registry ./internal/vault` | Existing files need edits | pending |
| 01-01-02 | 01 | 1 | LIC-04 | T-04 | README names Apache License 2.0 and links the root license. | release check | `rg -n '^## License$' README.md && rg -n 'Apache License 2\.0.*\[LICENSE\]\(LICENSE\)' README.md` | Existing file needs edit | pending |
| 01-01-03 | 01 | 1 | LIC-01 | T-01 | Only Shreyansh Sancheti can affirm authority. Pending or uncertain authority blocks execution. | blocking human checkpoint | Exact status, reviewer, and real calendar date are entered personally. | Authority record created by Task 1 | pending |
| 01-01-04 | 01 | 1 | LIC-01 | T-01 | The exact 2-field affirmation change is persisted once with DCO sign-off. | committed blob check | `git show HEAD:docs/relicense-authorization.md` plus exact parent-to-HEAD reconstruction and `git show --check --oneline HEAD` | Authority record created by Task 1 | pending |
